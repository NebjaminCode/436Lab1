...
const Product = class Product {
    constructor(input) {
        // saving local copy of string
        this.input = input;
        // splitting local copy on comma, into an array
        const inputArr = input.split(",");
        // assigning parameters from array
        this.name = inputArr[0];
        this.price = Number(inputArr[1]);
        this.availability = inputArr[2];
    }
     ...
     */
    static inStock = (products) => { return products.filter(product => product.availability === "In Stock");}
     ...
     * Reference: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array 
     */
    static halfOff = (products) => { return products.map(product => new Product (product.name+","+(product.price*.5)+","+product.availability)) }
    ...
    static printProducts = (products) => {
        return products.forEach(product => console.log(`Product: ${product.name}, Cost: $${product.price.toFixed(2)}, Availability: ${product.availability === "In Stock" ? "Yes" : "No"}`))
    }
